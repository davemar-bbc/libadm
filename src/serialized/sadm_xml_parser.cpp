...
namespace adm {
  namespace xml {
    ...
    std::shared_ptr<Frame> SadmXmlParser::parse() {
      ...
      if (root) {
        for (NodePtr node = root->first_node(); node; node = node->next_sibling()) {
   ...
   std::shared_ptr<AudioProgramme> SadmXmlParser::parseAudioProgramme(
    ...
    std::shared_ptr<AudioContent> SadmXmlParser::parseAudioContent(NodePtr node) {
      // clang-format off
    ...
    std::shared_ptr<AudioTrackUid> SadmXmlParser::parseAudioTrackUid(NodePtr node) {
      // clang-format off
